<?php
		...
		if( isset( $_GET['SULlyDeleteAction'] ) && array_key_exists( '_wpnonce', $_GET ) && wp_verify_nonce( $_GET['_wpnonce'], 'delete_old' ))
			{
			if( !isset( $_GET['SULlyActionsDeleteOld'] ) ) { $_GET['SULlyOptionsDeleteOld'] = $deletedays; }

			$deletedays = intval( $_GET['SULlyActionsDeleteOld'] );
			...
			$temp = sprintf( __( "%d records over %d days old have been deleted.", 'sully' ), $NumRows, $deletedays );
		...
		if( isset( $_GET['SullyReRunUnknownAction'] ) && array_key_exists( '_wpnonce', $_GET ) && wp_verify_nonce( $_GET['_wpnonce'], 'reprocess' ))
			{
			$TableName = $wpdb->prefix . "SULly";
		...
		if( isset( $_GET['SULlyRecreateAction'] ) && array_key_exists( '_wpnonce', $_GET ) && wp_verify_nonce( $_GET['_wpnonce'], 'recreate_db' ))
			{
			SULlySetup();
		...
		if( array_key_exists( 'SULlyRemoveAction', $_GET ) && array_key_exists( '_wpnonce', $_GET ) && wp_verify_nonce( $_GET['_wpnonce'], 'remove_db' ) )
			{
			$TableName = $wpdb->prefix . "SULly";
		...
		if( array_key_exists( 'SULlyOptions', $_POST ) && isset( $_POST['SULlyUpdateOptions'] ) && array_key_exists( '_wpnonce', $_POST ) && wp_verify_nonce( $_POST['_wpnonce'], 'update_options' ))
			{
			...
			$SULlyUtils->update_option( 'EntriesToDisplay', intval( $_POST['SULlyOptions']['WidgetDisplayLines'] ) );
			$SULlyUtils->update_option( 'PageEntriesToDisplay', intval( $_POST['SULlyOptions']['PageDisplayLines'] ) );
			$SULlyUtils->update_option( 'SendEmailNotifications', intval( $_POST['SULlyOptions']['SendEmailNotifications'] ) );
			$SULlyUtils->update_option( 'DisableWPEmailNotifications', intval( $_POST['SULlyOptions']['DisableWPEmailNotifications'] ) );
...
?>
		...
		<div id="fragment-1">
			...
			<div>&nbsp;</div>
			<div><?php _e('Delete records older than ', 'sully'); ?>:&nbsp;<input name="SULlyActionsDeleteOld" type="text" id="SULlyActionsDeletOld" size="3" maxlength="3" value="<?php echo $deletedays; ?>" /> <?php _e('days', 'sully'); ?> <input type="button" id="SullyDeleteAction" class="button-primary" name="SULlyDeleteAction" value="<?php _e('Delete'); ?>" onclick="if( confirm('<?php _e('Ok, last chance, really delete records over', 'sully'); ?> ' + document.getElementById('SULlyActionsDeletOld').value + '<?php _e(' days?', 'sully'); ?>') ) { window.location = '<?php echo wp_nonce_url('options-general.php?page=sully.php&SULlyDeleteAction=TRUE', 'delete_old' ); ?>' + '&SULlyActionsDeleteOld=' + document.getElementById('SULlyActionsDeletOld').value; }"/></div>
			<div>&nbsp;<hr />&nbsp;</div>
			<div><?php _e('Re-process entries listed as Unknown or Failed', 'sully'); ?>:&nbsp;<input type="button" id="SullyReRunUnknownAction" class="button-primary" name="SULlyReRunUnknownAction" value="<?php _e('Reprocess Unknown', 'sully'); ?>" onclick="window.location = '<?php echo wp_nonce_url( 'options-general.php?page=sully.php&SullyReRunUnknownAction=TRUE', 'reprocess'); ?>';"/></div>
...
?>
			...
			<div>&nbsp;</div>
			<div><?php _e('Remove the database table and all settings:', 'sully')?>&nbsp;<input type="button" class="button-primary" id="SullyRemoveAction" name="SULlyRemoveAction" value="<?php _e('Remove'); ?>" onclick="if( confirm('<?php _e('Ok, last chance, really remove the database table?'); ?>') ) { window.location = '<?php echo wp_nonce_url('options-general.php?page=sully.php&SULlyRemoveAction=TRUE','remove_db');?>'}"/></div>
<?php
		}
	else
		{
?>
			<div><?php _e('Recreate database table and settings:', 'sully')?>&nbsp;<input type="button" id="SullyRecreateAction" name="SULlyRecreateAction" value="<?php _e('Recreate', 'sully') ?>" onclick="window.location = '<?php echo wp_nonce_url('options-general.php?page=sully.php&SULlyRecreateAction=TRUE', 'recreate_db');?>'"/></div>
<?php
		}
?>
